...
function _update (oldVnode, vnode) {
  ...
  const oldDirs = normalizeDirectives(oldVnode.data.directives, oldVnode.context) // 从旧节点中获取规范化后的指令
  ...
  */
  ...
  let key, oldDir, dir
  for (key in newDirs) { // 遍历所有新的指令
    // 获取新老的指令项
    oldDir = oldDirs[key]
    dir = newDirs[key]
    if (!oldDir) { // 老节点没有该指令，进行绑定
      // new directive, bind
...
function callHook (dir, hook, vnode, oldVnode, isDestroy) {
  const fn = dir.def && dir.def[hook] // 获取指令中的钩子处理回调
  if (fn) { // 如果有则执行
    try {
